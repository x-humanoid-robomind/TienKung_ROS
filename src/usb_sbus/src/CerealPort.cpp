...
int cereal::CerealPort::write(const char * data, int length, int delay_time)
{
	usleep(delay_time);
...
bool cereal::CerealPort::readBetween(std::string * buffer, char start, char end, int timeout)
{
	...
	while(buffer->size() < buffer->max_size()/2)
	{
		...
		ret = buffer->find_last_of(end);

		if(ret > 0 && (int(buffer->at(1)) <= (buffer->size() - 3)))
		{
			// If it is there clear everything after it and return
...
bool cereal::CerealPort::readBetween_weixue(std::string * buffer, char start, char end, int timeout)
{
	int ret;
	...
	while(buffer->size() < buffer->max_size()/2)
	{
		...
		if(ret > 0 && (int(buffer->at(1) & 0x0f) <= (buffer->size() - 5)))
		{
			// If it is there clear everything after it and return
			erased = buffer->substr(ret+1, buffer->size()-ret-1);
			//std::cout << "sobra |" << erased << "|\n";
