...
namespace cereal
{
	...
	class CerealPort
	{
		...
		*  \param delay_time  write data after delay time , unit is microsecond(us). Default is 1 ms. 
		*
		*  \return Number of bytes writen.
		*/
		int write(const char * data, int length = -1, int delay_time = 1 * 1000);
		...
		*/
		//TODO: int readBetween(char * data, int length, char start, char end, int timeout = -1);
	    bool readBetween_weixue(std::string * data, char start, char end, int timeout = -1);
