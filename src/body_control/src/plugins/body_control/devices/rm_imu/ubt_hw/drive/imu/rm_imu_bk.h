...
class RmImu : public ubt_hw::ImuBase {
 ...
 protected:
  void filterUpdate(const ros::Time &time, double *gryo, double *accel);
  void filterReset();
  void angularVelFilter(double *gryo, const ros::Time &time);

  double angular_vel_offset[3] = {-0.0022, 0.0001, -0.0001};
  ...
  std::vector<std::unique_ptr<ubt_hw::LowPassFilter>> gryo_filters_;
  double lp_freq = 50;
