...
#include "../curves/QuaternionSLERPCurve.hpp"
#include "InterpolatableQuaternionSpline.hpp"
#include <Eigen/StdVector>

namespace broccoli {
namespace curve {
    //! Class representing a quaternion spline as concatenation of quaternion SLERP curve segments
    /*!
     * \ingroup broccoli_curve_splines
     *
     * See \ref InterpolatableQuaternionSpline and \ref QuaternionSLERPCurve for details.
     */
    class QuaternionSLERPSpline : public QuaternionSpline<QuaternionSLERPCurve>, public InterpolatableQuaternionSpline {
    public:
        //! Constructor
        QuaternionSLERPSpline()
        {
        }

        //! Destructor
        virtual ~QuaternionSLERPSpline()
        {
        }
        ...
        virtual bool interpolate(const std::vector<Eigen::Quaterniond, Eigen::aligned_allocator<Eigen::Quaterniond>>& parameters, const std::vector<double>* const proportions, const QuaternionSplineInterpolationMethod& method, QuaternionSplineResult* const result = nullptr)
        {
            ...
            case QuaternionSplineInterpolationMethod::SLERP_PIECEWISE: {
                /*! Interpolation with SLERP_PIECEWISE
                 *  ----------------------------------
    ...
    protected:
        //! **Piecewise** interpolation with SLERP segments with given control points (see \ref interpolate() for details on \p parameters, \p proportions and \p result)
        /*!
         * Interpolate with <b>S</b>pherical <b>L</b>inear int<b>ERP</b>olation (SLERP) (see \ref QuaternionSLERPCurve) between start- and end-quaternions
